...
export function generateMetadata() {
  const post = blogPosts.find((p) => p.slug === "announcing-anthropic-partnership");
    ...
    <div className="flex flex-col min-h-screen bg-background pt-24">
        ...
        <div className="container mx-auto px-4 max-w-[800px] relative z-10">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 py-6 border-y border-border/50">
             <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-primary to-[#22D3EE] flex items-center justify-center text-lg font-bold text-foreground shadow-lg">
                  {post.authorAvatar}
                </div>
                <div>
             ...
             <div className="flex items-center gap-2">
                ...
                <button className="w-10 h-10 rounded-full bg-muted border border-border/50 flex items-center justify-center text-foreground hover:bg-primary hover:border-primary transition-all">
                  <Twitter size={18} />
                </button>
                <button className="w-10 h-10 rounded-full bg-muted border border-border/50 flex items-center justify-center text-foreground hover:bg-primary hover:border-primary transition-all">
                  <Linkedin size={18} />
                </button>
                <button className="w-10 h-10 rounded-full bg-muted border border-border/50 flex items-center justify-center text-foreground hover:bg-muted/80 transition-all">
                  <Copy size={18} />
                </button>
             </div>
          </div>
      ...
      <section className="pb-12">
         <div className="container mx-auto px-4 max-w-[1000px]">
            <div className="w-full aspect-[21/9] md:aspect-[2.5/1] rounded-2xl overflow-hidden border border-border/50">
        ...
        <div className="container mx-auto px-4 max-w-[800px]">
           ...
           <div className="mt-16 pt-8 border-t border-border/50">
             ...
             <div className="flex flex-wrap gap-2">
                {post.tags.map((tag, i) => (
                   <span key={i} className="px-3 py-1.5 rounded-lg bg-card border border-border text-muted-foreground text-sm hover:text-foreground hover:border-border/80 cursor-pointer transition-colors">
                      #{tag}
                   </span>
                ))}
             </div>
         ...
         <div className="container mx-auto px-4 max-w-[1200px]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               {blogPosts.filter(p => p.slug !== post.slug).slice(0, 3).map((relatedPost) => (
                 <Link href={`/blog/${relatedPost.slug}`} key={relatedPost.slug} className="group flex flex-col h-full bg-background rounded-2xl border border-border overflow-hidden hover:border-primary/30 transition-colors">
                  ...
                  <div className="p-6 flex flex-col flex-1">
                     <div className="text-primary text-xs font-bold uppercase tracking-wider mb-3">{relatedPost.category}</div>
