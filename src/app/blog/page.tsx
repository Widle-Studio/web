...
export default function BlogIndexPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pt-24">
           ...
           <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
              ...
              <div className="relative shrink-0">
                ...
                <input
                  ...
                  className="w-full md:w-64 bg-card border border-border/50 rounded-full pl-10 pr-4 py-2 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-[#6366F1]/50 transition-all"
                />
              </div>
           </div>
        </div>
      ...
      <section className="py-16 md:py-24">
        <div className="container mx-auto px-4 max-w-[1200px]">
           <Link href={`/blog/${featuredPost.slug}`} className="group flex flex-col lg:flex-row gap-12 lg:items-center">
              <div className="w-full lg:w-3/5 aspect-video rounded-3xl overflow-hidden border border-border/50 relative">
              ...
              <div className="w-full lg:w-2/5 flex flex-col">
                 ...
                 <h1 className="text-4xl md:text-5xl font-bold text-foreground leading-tight mb-6 group-hover:text-primary transition-colors">
                    {featuredPost.title}
                 </h1>
                 ...
                 <div className="mt-auto flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-muted/50 flex items-center justify-center text-sm font-bold text-foreground border border-border">
                        {featuredPost.authorAvatar}
                      </div>
                      <span className="text-foreground font-medium">{featuredPost.author}</span>
                    </div>
                    <span className="text-primary font-medium group-hover:translate-x-1 transition-transform flex items-center gap-2">
        ...
        <div className="container mx-auto px-4 max-w-[1200px]">
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {regularPosts.map((post) => (
                <Link href={`/blog/${post.slug}`} key={post.slug} className="group flex flex-col h-full bg-card rounded-2xl border border-border overflow-hidden hover:border-primary/30 transition-colors">
      ...
      {/* Newsletter */}
      <section className="py-24 bg-card border-t border-white/[0.06] mt-auto relative overflow-hidden">
        <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-br from-primary/10 to-transparent blur-[100px] rounded-full pointer-events-none" />
        <div className="container mx-auto px-4 max-w-3xl text-center relative z-10">
          ...
          <form className="flex flex-col sm:flex-row gap-4 max-w-lg mx-auto" onSubmit={(e) => e.preventDefault()}>
            <input
              ...
              required
              className="flex-1 bg-background border border-border/50 rounded-lg px-4 py-3 text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-[#6366F1]/50"
            />
            <button
              type="submit"
