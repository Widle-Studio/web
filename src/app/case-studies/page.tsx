          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {caseStudies.map((study) => (
              <Link
                ...
                key={study.id}
                className="group flex flex-col bg-card border border-border rounded-2xl overflow-hidden hover:border-primary/50 transition-all duration-300 hover:shadow-[0_8px_32px_rgba(99,102,241,0.15)] h-full"
              >
                {/* Thumbnail */}
                <div className="w-full aspect-[4/3] bg-gradient-to-br from-[#0D0F14] to-[#22D3EE]/10 relative border-b border-border overflow-hidden flex items-center justify-center p-8">
                  {/* Abstract graphic */}
                  <div className="w-full h-full bg-card rounded-lg border border-border/50 shadow-2xl relative p-4 group-hover:scale-[1.05] transition-transform duration-500">
                ...
                <div className="p-8 flex-1 flex flex-col">
                  <div className="flex flex-wrap gap-2 mb-6">
                    {study.tags.slice(0, 3).map((tag, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 bg-muted text-muted-foreground text-xs font-medium tracking-wide rounded-full border border-border/50"
                      >
                        {tag}
                      </span>
                    ))}
