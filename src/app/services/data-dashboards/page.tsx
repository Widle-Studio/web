...
export function generateMetadata() {
  const service = services.find((s) => s.id === "data-dashboards");
...
export default function DataDashboardsPage() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background pt-24">
      ...
      <section className="py-20 md:py-32 relative overflow-hidden border-b border-border">
        ...
        <div className="container mx-auto px-4 max-w-[1000px] relative z-10">
          ...
          <div className="flex flex-col md:flex-row gap-12 items-start md:items-center">
            <div className="w-24 h-24 rounded-2xl bg-foreground/5 flex items-center justify-center border border-border shrink-0">
               <Activity className="w-12 h-12 text-[#6366F1]" />
            </div>
            <div>
        ...
        <div className="container mx-auto px-4 max-w-[1000px] text-center">
            ...
            <div className="flex flex-wrap justify-center gap-4">
                {service.tools?.map((tool, index) => (
                    <span key={index} className="px-6 py-3 bg-background border border-border rounded-xl text-foreground font-medium shadow-sm hover:border-[#6366F1]/50 transition-colors">
                        {tool}
                    </span>
                ))}
            </div>
      ...
      {/* CTA Bottom */}
      <section className="py-24 border-t border-border text-center mt-auto">
        <div className="container mx-auto px-4">
          ...
          <Link
            href="/contact"
            className="inline-flex items-center gap-2 px-8 py-4 rounded-lg bg-gradient-to-r from-[#6366F1] to-[#8B5CF6] text-primary-foreground font-medium hover:opacity-90 transition-opacity"
          >
