...
export default function ServicesGrid() {
  ...
  return (
    <section className="py-24 bg-card border-y border-white/[0.06]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
          {services.map((service, index) => {
            ...
            return (
              <div
                key={service.id}
                className="group relative p-8 rounded-2xl bg-background border border-border hover:border-border/50 transition-all duration-300 hover:shadow-[0_8px_32px_rgba(99,102,241,0.15)] flex flex-col h-full"
              >
                <div className="w-14 h-14 rounded-xl bg-muted flex items-center justify-center mb-8 group-hover:bg-primary/10 transition-colors border border-border/50">
                ...
                <div className="space-y-6">
                  <div className="flex flex-wrap gap-2">
                    {service.tools.slice(0, 4).map((tool, i) => (
                      <span
                        key={i}
                        className="text-xs font-medium px-2.5 py-1 rounded-full bg-muted text-foreground/70 border border-border/50"
                      >
                        {tool}
                      </span>
                    ))}
                    {service.tools.length > 4 && (
                      <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-muted text-foreground/50 border border-border/50">
